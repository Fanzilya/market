...
export default function LandingPage() {
  ...
  const testimonials = [
    { name: 'Алексей Иванов', role: 'Главный инженер, ГУИС', text: 'Платформа значительно упростила процесс сбора коммерческих предложений. Раньше уходили недели, теперь всё автоматизировано.', rating: 5 },
                ...
                {Array.from({ length: Math.ceil(partners.length / 4) }).map((_, slideIndex) => (
                  <div key={slideIndex} className={styles.carouselSlide}>
                    {partners.slice(slideIndex * 4, slideIndex * 4 + 4).map((partner) => (
                      <div key={partner.id} className={styles.partnerCard} onClick={() => handlePartnerClick(partner.slug)}>
